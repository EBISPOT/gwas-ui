...
Each GWAS Catalog data release is also available in OWL/RDF format from link:https://ftp.ebi.ac.uk/pub/databases/gwas/releases/2025/01/31/[ our FTP server]. The Catalog data can be found in the gwas-kb.owl file while gwas-diagram.owl contains the schema ontology defining the relationships between different Catalog concepts. Please note that the knowledge base file is very large. For this reason, it excludes some Catalog data, such as sample descriptions and ancestry information.
