...
The GWAS Catalog contains SNP-by-SNP and SNP-by-environment interaction studies as long as the SNPs analysed meet our link:methods/criteria-content[criteria] of being genome-wide. For both types of study, the term “interaction” is included in the reported trait.
...
The GWAS Catalog website does not currently have a way to view all associations for a particular gene or genomic region. We are currently working to improve this functionality. For now, we recommend using our link:api-content[REST API] to return associations for a specific gene or genomic region.
...
These tables can be found on the each of the specific "Trait", "Variant" and "Publication" pages. The data displayed is highly customisable. You can refine the results by typing into a) the search box above the table, to display all rows containing your search term in any column, or b) the filter boxes at the top of each column to search only within that column. A set of columns are displayed by default, but you can also view extra columns or hide unnecessary columns using the "Add/Remove Columns" button. You can sort by any column by clicking on the column header, or use the "Multiple Sort" button for more complex multi-level sorting. Finally, you can use the "Export data" button to download the table as a csv file. Note that the csv file will be updated to match any changes you have made to the rows, columns or sorting.
