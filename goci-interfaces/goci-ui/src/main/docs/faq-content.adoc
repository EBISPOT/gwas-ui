...
// Related search terms: search, query, publication search, trait search, variant search, filter, refine results
...
// Related search terms: publication search, PubMed ID, author search, title search, publication results
...
// Related search terms: author search, trait overlap, publication search, text match, PubMed ID
...
// Related search terms: variant search, SNP, rsID, genomic region, gene mapping, Ensembl annotation
...
// Related search terms: gene search, gene page, reported gene, mapped gene, REPORTED GENE(S), REST API, curation cutoff 2021
...
// Related search terms: region search, chromosomal coordinates, cytogenetic band, SNP location, genomic interval
...
If you cannot find your trait of interest, it may be stored under a synonym. For example, searching “general cognitive ability” returns “intelligence,” which is the standardized trait name in the Catalog.  

The search bar also offers auto-suggestions, including synonyms, as you type.  

// Related search terms: trait search, phenotype, synonym, child term, ontology, autocomplete
...
// Related search terms: trait search, publication results, study table, PubMed ID, trait page
'''
...
// Related search terms: trait search, child trait, ontology, synonyms, compound trait, EFO
'''
...
// Related search terms: compound trait, multiple traits, reported trait, ontology, UK Biobank, ICD codes
'''
...
// Related search terms: background trait, main trait, study context, spreadsheet, API, asthma, allergic rhinitis
'''
...
* Use the autocomplete suggestions offered in the search bar.  
...
// Related search terms: no results, spelling error, synonyms, autocomplete, EFO
'''
=== B(iv). Searching by study
...
// Related search terms: study search, GCST, accession number, study table
'''
...
// Related search terms: download associations, studies table, TSV, CSV, export, file downloads
...
// Related search terms: association plot, trait page, visualisation, LocusZoom, genome-wide view, download plot
...
// Related search terms: LD plot, linkage disequilibrium, r2, D’, Ensembl, variant page, REST API, download LD data
...
// Related search terms: studies table, associations table, filter, column visibility, sort, export csv, download

== [[D]]D. Diagram

==== [[faqD1SnpInfo]] How do I display SNP details for a dot in the diagram?

Click any dot (coloured circle) in the diagram to view all SNPs associated with that location and trait.  

The pop-up shows:  
* SNP IDs  
* p-values  
* Study information  
* Trait names (curated EFO terms)  

All fields in the pop-up are clickable and link to the corresponding page in the Catalog. Icons provide external links to Ensembl, EFO, or Europe PMC for more details.  

Close the pop-up by clicking outside it, the “×” in the corner, or the “Close” button.  

// Related search terms: diagram, SNP info, interactive pop-up, variant details, EFO term, study link

==== [[faqD2FilterDiagram]] How do I filter the diagram?

Use the search box next to the diagram to filter by trait:  
* Start typing a trait name — after 3–4 characters, auto-complete suggestions from EFO will appear.  
* Select a suggestion and click *Apply* or press Enter.  

When filtered, only dots for the selected trait remain highlighted. Other traits are faded, and a counter shows how many associations match your search.  

Note: The filter uses EFO trait names and synonyms, which may differ slightly from author-reported trait names in publications.  

// Related search terms: diagram filter, trait search, EFO suggestions, highlight associations

==== [[faqD3DiagramLegend]] How do I display and use the legend?

The legend is displayed in the left-hand sidebar of the diagram. It shows:

* The colour scheme used for trait categories.
* A count of the number of associations (dots) in each category.

You can:

* Click the chevron icon at the top of the sidebar to hide the legend and maximise space for the diagram.
* Click a category in the legend to filter the diagram by that category (not available for "other" categories like *other disease* or *other trait*).

Note: Some traits belong to multiple categories (for example, Crohn’s disease is classified as both a digestive system and immune system disease). Each association is assigned a single colour based on the most specific EFO ancestor term, so you may see different colours for traits with overlapping categories.  

// Related search terms: diagram legend, colour categories, category filter, trait classification

==== [[faqD4DiagramLabels]] How do I display labels?

Labels appear when you hover your mouse over elements in the diagram:

* **Chromosome labels:** Display the chromosome number or region.
* **Trait labels (coloured circles):** Show the trait name (EFO term) and its position.

These labels help you identify which trait or region each point represents without leaving the diagram.  

// Related search terms: diagram labels, hover, chromosome label, trait label, EFO term

==== [[faqD5DiagramZoomMove]] How do I zoom and move the diagram?

The GWAS Catalog diagram supports Google Maps–style navigation:

* **Zoom:**  
  * Scroll up to zoom in, scroll down to zoom out.  
  * Alternatively, use the zoom bar in the top-right corner — drag the slider or click the + and − buttons.  

* **Pan (move):**  
  * Click and hold the left mouse button, then drag the diagram to reposition the view.  
  * This is especially useful for centring on a specific region at higher zoom levels.  

// Related search terms: diagram zoom, pan, scroll, drag, navigation, move view

==== [[faqD6DiagramCitation]] How should I cite the diagram?

Cite the diagram using the same format as for the GWAS Catalog itself.  
Full citation guidance is available on the https://www.ebi.ac.uk/gwas/docs/about[About page].

// Related search terms: diagram citation, figure reference, GWAS Catalog diagram, how to cite

==== [[faqD7DiagramDownload]] How do I download the diagram?

High-resolution images of the GWAS Catalog diagram can be downloaded in several formats (e.g. PNG, SVG, PDF).  
Download options are listed on the https://www.ebi.ac.uk/gwas/docs/diagram-downloads[diagram downloads page].

// Related search terms: diagram download, high-resolution image, PNG, SVG, PDF, save diagram

==== [[faqD8FilteredDiagramDownload]] Can I download a filtered version of the diagram?

Currently, the GWAS Catalog does not provide a built-in option to download a diagram that is filtered by trait.  

*Quick option:*  
Take a screenshot of the filtered diagram if a high-resolution image is not required.

*Advanced workaround (for high-resolution SVG):*  
1. Open the diagram in Firefox (similar steps work in other browsers).  
2. Right-click on the filtered diagram and select **Inspect element**.  
3. In the Inspector, hover over the `<svg>` element, right-click, and choose **Copy → Outer HTML**.  
4. Paste into a text editor and save the file with a `.svg` extension.  
5. Open the file in a vector graphics program (e.g. Inkscape, Illustrator) and export to your preferred format.  

// Related search terms: filtered diagram download, SVG export, high resolution, screenshot, Firefox inspector
'''
...
// Related search terms: SNP mapping, genomic annotation, Ensembl pipeline, gene mapping, intergenic SNP, upstream gene, downstream gene, genome build
'''

==== [[faqE2GenomeBuild]] Which genome build is used in the GWAS Catalog?

All GWAS Catalog data is mapped to the human reference genome assembly **GRCh38.p14** and **dbSNP build 156**.  

This information is updated on release and displayed at the bottom of the GWAS Catalog's home page.

// Related search terms: genome build, GRCh38, dbSNP build 144, reference genome, assembly version
'''
...
// Related search terms: alternative genome builds, GRCh37, liftover, Ensembl REST API, NCBI Remap, assembly mapping, rsID conversion
'''
...
// Related search terms: missing SNP mapping, unplaced SNP, dbSNP retired, unmapped variant, genome build issues, SNP identifier error
'''
...
// Related search terms: SNP interaction display, haplotype format, rsID separator, multi-SNP annotation, x separator, semicolon separator, mapped gene alignment
'''
...
// Related search terms: most severe consequence, variant effect, VEP, Ensembl consequence ranking, transcript annotation, functional effect
'''
...
// Related search terms: risk allele mismatch, strand flipping, genome build differences, allele orientation, reference genome discrepancy
'''
...
// Related search terms: population descriptors, ancestry category, sample metadata, population standardisation, controlled vocabulary
'''
...
// Related search terms: population search, ancestry search, discovery sample, replication sample, column visibility, sample metadata
'''
...
// Related search terms: ancestry label search, population filter, ancestry download file, REST API ancestry, sample metadata
'''
...
// Related search terms: pre-2011 ancestry, double-curation, population descriptors, curation quality, metadata review
'''
...
// Related search terms: ancestry spreadsheet, sample metadata, discovery stage, replication stage, ancestry file format, multi-dimensional data
'''
...
// Related search terms: cohort field, GWAS cohorts, discovery stage, PGS Catalog, cohort abbreviations, NR, other, missing cohort info
'''
...
// Related search terms: GWAS API, REST API, programmatic access, JSON output, query by trait, query by variant, GCST lookup
'''
...
// Related search terms: summary statistics, full GWAS results, complete dataset, effect sizes, p-values, meta-analysis, polygenic risk score, GWAS download
'''
...
// Related search terms: summary statistics availability, studies with summary stats, FTP site, early release, unpublished data, association count icon
'''
...
* **Programmatic access:**  
Use the dedicated summary statistics REST API for searchable, filterable, and harmonised data:  
http://www.ebi.ac.uk/gwas/summary-statistics/docs/  

* **Direct download:**  
Access the original, standardised, and harmonised files via the FTP site:  
http://ftp.ebi.ac.uk/pub/databases/gwas/summary_statistics/  
You can also find links to these files from the search interface and the  
https://www.ebi.ac.uk/gwas/downloads/summary-statistics[list of studies with summary statistics files].  

// Related search terms: summary statistics access, REST API, FTP download, harmonised data, programmatic access, search interface links
'''
...
// Related search terms: summary statistics citation, GCST accession, download date, publication reference, citing GWAS data
'''
...
// Related search terms: standardised summary stats, harmonised data, file format, meta-analysis, consistent columns, data cleaning
'''
...
// Related search terms: privacy risk, participant re-identification, Homer et al. 2008, allele frequency privacy, NIH guidance, open data sharing
'''
...
// Related search terms: CC0, data license, public domain, summary statistics license, EBI terms of use, data sharing policy
'''
...
// Related search terms: summary statistics submission, unpublished GWAS, deposition page, file format requirements, eligibility criteria
'''
...
// Related search terms: summary statistics format, file specification, validation tool, submission requirements, data standardisation
