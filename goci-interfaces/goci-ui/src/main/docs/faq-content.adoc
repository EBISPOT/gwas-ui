...
The GWAS Catalog <<E,diagram>> presents a graphical view of the GWAS Catalog data.

We also provide REST API access to the GWAS Catalog data. See <<H,Programmatic access below>> for more information.

From 2017, the GWAS Catalog started hosting summary statistics files in addition to curated associations. See <<I,Summary statistics below>> for more information.
...
Type your query, e.g. “breast carcinoma”, into the search box and hit return or click the search icon. You can type any text you wish into the search bar. The search then returns any *publications* (marked with the letter P), *variants* (V) or *traits* (T) in the Catalog that contain an exact string match within a number of data fields. You can use the “Refine search results” box on the left to show only publications, variants or traits. For more details on how to search for each specific document type see link:____
...
The GWAS Catalog website does not currently have a way to view all associations for a particular gene or genomic region. We are currently working to improve this functionality. For now, we recommend using our REST API to return associations for a specific gene or genomic region. Example scripts are available at... 
...
The search results may also contain traits that have been studied together with your trait of interest in some way, for example in a GWAS for multiple traits or for a compound trait. For example, searching for “asthma” also returns the trait “response to bronchodilator”. This is because the GWAS Catalog includes a study on response to bronchodilator in a sample of people who all have asthma. See <<faq-B9, FAQ B9>> to find out how more complicated phenotypes are represented in the Catalog.
...
The GWAS Catalog REST API is now available for programmatic access to the Catalog. See the link:http://www.ebi.ac.uk/gwas/docs/api[full technical documentation] here.
