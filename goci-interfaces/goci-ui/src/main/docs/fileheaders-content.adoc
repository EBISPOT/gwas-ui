...
UPSTREAM_GENE_DISTANCE^*^: distance in bp for nearest upstream gene to rs number, if not within gene

DOWNSTREAM_GENE_DISTANCE^*^: distance in bp for nearest downstream gene to rs number, if not within gene
...
GxE+: Study design is genome-wide genotype-by-environment interaction
