...
* We request that you submit files that correspond to the final QC’d version of your dataset, including only data which is usable in downstream analyses (e.g. applying filters for MAF, imputation quality, variant present in n cohorts, as you would do in your own research). We do not apply any of these filters post-submission.
