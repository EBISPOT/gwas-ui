...
==== Option 1: Upload files using Globus (...might need to add screenshots as in Simon's instructions...?)
...
* Complete the template. There are five columns for you to fill in: the highlighted columns are mandatory, while the grey columns are optional, although we encourage you to submit as much information as you can. See example[link].
...
|The summary statistics filename (for Globus upload) or full URL + filename (for a file hosted elsewhere online). Leave blank for any studies without summary statistics.
|no
...
|yes
|Must match one of the following options
|e.g. `Japan`. For a complete list of countries, please see [link to CIA Factbook, or other information source???]
|===
