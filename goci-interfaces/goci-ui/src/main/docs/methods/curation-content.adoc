...
Published GWA studies are identified through a weekly PubMed search.  Since 2018 we have employed a machine learning assisted triage system, whereby studies are identified by a convolutional neural network based classifier.  Subsequently studies are assessed for eligibility by human curators using a bespoke Pubtator triage interface.  The development of this method is described in Lee et. al., 2018 (PMID 30102703).  Prior to 2018 we used a query based method (searching the terms "genome-wide" OR "genome wide" OR "genome-wide" OR "GWAS" in Pubmed).
