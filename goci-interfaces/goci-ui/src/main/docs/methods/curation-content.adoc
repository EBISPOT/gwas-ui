...
When multiple traits are analysed in the same publication multiple entries are created, or (for older publications) individual SNPs are annotated with their specific traits. Each trait is annotated with one or more https://www.ebi.ac.uk/gwas/docs/ontology[ontology terms] which are used to summarise, query and visualise the data in the Catalog's interfaces.
...
The GWAS Catalog contains at least one entry (study) for each publication.
...
Full details of the methods and rationale used to represent population data in the GWAS Catalog can be found in our publication by link:http://rdcu.be/G6Fv[Morales et al.] and on our https://www.ebi.ac.uk/gwas/docs/population-descriptors[Population Descriptors] page.
