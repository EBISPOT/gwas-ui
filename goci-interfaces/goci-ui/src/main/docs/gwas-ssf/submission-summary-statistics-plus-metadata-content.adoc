...
You should create a new row for each ancestry category. Providing ancestry-specific sample numbers is important for reusability of the data. However, if separate sample numbers are unavailable for each category, you may enter multiple categories in the same row, separated by pipes "\|".
